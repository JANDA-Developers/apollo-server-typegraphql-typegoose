...
import { isClass } from "../filter/generateFilterType";
...
const generateAddFieldFunc =
 (typeGraphQLMetadata: MetadataStorage, sortContainer: any) =>
 (t: Function, name?: string) => {
  ...
  for (const { field, getReturnType } of sortingData) {
   ...
   const fieldName = graphQLField ? graphQLField.schemaName : field.toString();
   ...
   if (isClass(baseReturnType) && !name) {
    ...
    const sortName = `${name ? name.concat("_") : ""}${fieldName}${DIVIDER}`;
